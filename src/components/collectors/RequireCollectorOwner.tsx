"use client";

import Link from "next/link";
import { useCollectorData } from "./CollectorDataProvider";

export function RequireCollectorOwner({ children }: { children: React.ReactNode }) {
  const { role } = useCollectorData();

  if (role !== "owner") {
    return (
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-3 px-5 py-14 text-center">
        <span className="text-4xl">🔒</span>
        <p className="text-base font-bold text-neutral-900">هذه الصفحة للمالك فقط</p>
        <p className="text-sm text-neutral-400">
          السائقون يقدرون فقط يشوفون طلبات السحب ويؤكدون الاستلام
        </p>
        <Link href="/collectors" className="mt-2 text-sm font-bold text-brand-700">
          رجوع للطلبات ←
        </Link>
      </div>
    );
  }

  return <>{children}</>;
}
