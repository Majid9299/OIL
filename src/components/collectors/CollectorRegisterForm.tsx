...
export function CollectorRegisterForm() {
  const {
    registration,
    updateRegistration,
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col gap-6 px-5 py-8">
      <div>
        <h1 className="text-lg font-extrabold text-neutral-900">تسجيل بيانات شركة التجميع</h1>
        <p className="text-sm text-neutral-400">
          مطلوبة لاستخراج تصريح تجميع زيوت الطبخ المستعملة من هيئة البيئة
        </p>
      </div>

      <div className="flex flex-col gap-4 rounded-2xl border border-neutral-200 bg-white p-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-neutral-500">اسم الشركة</label>
          <input
            value={form.companyName}
            onChange={(e) => setForm((f) => ({ ...f, companyName: e.target.value }))}
            placeholder="مثال: شركة الخليج لتجميع الزيوت"
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-4">
        ...
        <p className="mb-3 text-xs text-neutral-400">
          أرفق نسخة الرقم الضريبي والسجل التجاري وتصريح هيئة البيئة
        </p>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-4">
        <p className="mb-1 text-sm font-bold text-neutral-700">نطاق التغطية الجغرافي</p>
        <p className="mb-3 text-xs text-neutral-400">
          اختر المحافظات والولايات التي تغطيها شركتك ({coverageWilayats.length} ولاية مختارة)
        </p>
        <CoverageAreaSelector selected={coverageWilayats} onToggle={toggleCoverageWilayat} />
      </div>
